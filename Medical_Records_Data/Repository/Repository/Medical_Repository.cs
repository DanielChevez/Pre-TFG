...
{
    public class Medical_Repository : IMedical_Conditions
    {

        private readonly DataBaseConnection _context;

        public Medical_Repository(DataBaseConnection context) { _context = context; }


        public async Task<JsonArray> Add_Habit(HabitoDTO obj)
        {
            ...
            try
            {
                ...
                await _context.Habito.AddAsync(habito);
                int result = await _context.SaveChangesAsync();

                if (result > 0)
                {

                    msgDb = "Se agregó correctamente!";
                    res = true;
                    resultsDB.Add(msgDb);
                    resultsDB.Add(res);
                }
        ...
        public async Task<JsonArray> Delete_Habit(int IdHabit)
        {
            return null;
